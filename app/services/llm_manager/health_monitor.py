...
class HealthMonitor:
    ...
    _CFG = get_config("health_monitor", {})
    SCORE_WEIGHTS = _CFG.get("score_weights", {
        "success_rate": 0.40,
        "latency": 0.30,
        "rate_limit": 0.20,
        "recency": 0.10
    ...
    UNHEALTHY_THRESHOLD = _CFG.get("unhealthy_threshold", 30)
    DEGRADED_THRESHOLD = _CFG.get("degraded_threshold", 60)
    LATENCY_IDEAL_MS = _CFG.get("latency_ideal_ms", 2000)
    LATENCY_MAX_MS = _CFG.get("latency_max_ms", 30000)
    ...
    def _calculate_score(self, metrics: ProviderMetrics) -> int:
        ...
        if metrics.requests_total == 0:
            ...
            rate_limit_score = 100 * (1 - min(rate_limit_ratio * 5, 1.0))
