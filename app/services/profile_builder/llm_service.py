...
class LLMService:
    ...
    async def get_exact_token_count(self, messages: list, ctx_label: str = "") -> Optional[int]:
        ...
        try:
            ...
            # v11.1: Usar novas variáveis (MODEL_KEY, URL_MODEL, MODEL_NAME) com fallback para VLLM_*
            api_key = config.MODEL_KEY or config.VLLM_API_KEY
            base_url = config.URL_MODEL or config.VLLM_BASE_URL
            model = config.MODEL_NAME or config.VLLM_MODEL
